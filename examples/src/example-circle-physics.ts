...
const strokeWidth = g.mix(g.f32(0.75 * devicePixelRatio), g.f32(1.25 * devicePixelRatio), vPointer);
...
const discs = new g.Mesh(
    ...
    new g.Material({
        vertex: clip,
        fragment: g.vec4(
            g.mix(ink(palette.base), g.select(grey(g.f32(0.65)), ink(ACCENT), g.greaterThanEqual(vPointer, g.f32(0.5))), stroke),
